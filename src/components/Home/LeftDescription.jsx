...
const LeftDescription = ({ data, KeyData }) => {
  if (!data) return <p>Loading.......</p>;

  return (
    <div className='my-2'>
      {data.map((item, index) => (
        <div key={index} className={`grid gap-${window.innerWidth < 640 ? 2 : 4} mb-8`}>
            ...
            <div className=' hidden lg:flex flex-col lg:flex-row lg:space-x-10'>
              <div className="w-full md:w-1/2">
                <HeadTitle heading={item.title} number={item.number} />
                <DescriptionComponent description={item.description} />
                <div className="max-w-full md:max-w-2xl flex justify-between flex-wrap">
                  <ArrowButton btnName={KeyData} buttonLink={item.buttonLink} />
                </div>
              </div>
